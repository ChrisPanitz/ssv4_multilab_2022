...
# --- content: plotting single-trial SNR across trials (common pipeline, single-trial FFT, single-trial SNR)
...
yminFL <- min(dfTimeCourse$SNR_smoothed[dfTimeCourse$lab == "Florida"])
ymaxFL <- max(dfTimeCourse$SNR_smoothed[dfTimeCourse$lab == "Florida"])
yminLE <- min(dfTimeCourse$SNR_smoothed[dfTimeCourse$lab == "Leipzig"])
ymaxLE <- max(dfTimeCourse$SNR_smoothed[dfTimeCourse$lab == "Leipzig"])

timeCourseFL6 <- ggplot(dfTimeCourse[dfTimeCourse$lab == "Florida" & dfTimeCourse$freq == "6Hz",], 
                        aes(x = trial, y = SNR_smoothed, color = mod)) + theme_classic() +
  geom_line() +
  scale_color_manual(values = brewer.pal(4,"Purples")[4:3]) +
  scale_y_continuous(limits = c(yminFL,ymaxFL), name = "ssVEP SNR") +
...
timeCourseLE6 <- ggplot(dfTimeCourse[dfTimeCourse$lab == "Leipzig" & dfTimeCourse$freq == "6Hz",], 
  ...
  geom_line() +
  scale_color_manual(values = brewer.pal(4,"Purples")[4:3]) +
  scale_y_continuous(limits = c(yminLE,ymaxLE), name = "ssVEP SNR") +
  scale_x_continuous(name = "trial #") +
...
# Save figure
savename = paste0(parentFolder,"/figures/28b_supp_SNRAcrossTrials.pdf")
ggsave(filename = savename, plot = timeCourses, device = "pdf",
       width = 21, height = 15, unit = "cm", limitsize = FALSE)

savename = paste0(parentFolder,"/figures/28b_supp_SNRAcrossTrials.jpg")

...
# --- content: plotting PLI group stats for different conditions in aggregated sample
...
#   scale_x_continuous(name = " ", limits = c(xminSpec,xmaxSpec), breaks = seq(6,xmaxSpec,6), labels = as.character(seq(6,xmaxSpec,6))) +
...
#   scale_x_continuous(name = " ", limits = c(xminSpec,xmaxSpec), breaks = seq(8.57,xmaxSpec,8.57), labels = as.character(seq(8.57,xmaxSpec,8.57))) +
...
#   scale_x_continuous(name = " ", limits = c(xminSpec,xmaxSpec), breaks = seq(15,xmaxSpec,15), labels = as.character(seq(15,xmaxSpec,15))) +
#   scale_y_continuous(name = " ", limits = c(yminSpec,ymaxSpec)) +
...
# set y axis limits
ymin = min(dfStats$pliZ)
ymax = max(dfStats$pliZ)
...
for (i in 1:length(levels(dfStats$part))) {
  dfStatsWithin$pliZ[dfStats$part == levels(dfStats$part)[i]] <- 
    dfStats$pliZ[dfStats$part == levels(dfStats$part)[i]] - 
    mean(dfStats$pliZ[dfStats$part == levels(dfStats$part)[i]]) + 
    mean(dfStats$pliZ)
}
...
plot6 <- ggplot() + theme_classic() +
  geom_line(data = dfStats[dfStats$freq == "6Hz",], 
            aes(x = mod, y = pliZ, group = part), color = "gray70") +
  geom_point(data = dfStats[dfStats$freq == "6Hz",], 
             aes(x = mod, y = pliZ), color = "gray70") +
  ...
  scale_y_continuous(name = " Phase-Locking Index (z)", limits = c(ymin,ymax)) +
  labs(title = " ") +
  theme(
    plot.title = element_text(size = fSize, color = "black", face = "bold", hjust = .5),
...
dfStatsDiffAmp <- pivot_wider(data = dfStats, id_cols = c(part,lab,freq), names_from = c(mod), values_from = c(pliZ))
...
# make histograms
hist6 <- ggplot(data = dfStatsDiffAmp[dfStatsDiffAmp$freq == "6Hz",],
...
hist857 <- ggplot(data = dfStatsDiffAmp[dfStatsDiffAmp$freq == "8.57Hz",],
                     aes(x = diff)) + theme_classic() +
  geom_histogram(breaks = histBreaks, color = inlayFill, fill = "gray20") +
  scale_x_continuous(breaks = xbreaks) +
  scale_y_continuous(limits = c(yminHist,ymaxHist), breaks = c(), expand = c(0,0,0,1)) +
...
hist15 <- ggplot(data = dfStatsDiffAmp[dfStatsDiffAmp$freq == "15Hz",],
                    aes(x = diff)) + theme_classic() +
  geom_histogram(breaks = histBreaks, color = inlayFill, fill = "gray20") +
  scale_x_continuous(breaks = xbreaks) +
  scale_y_continuous(limits = c(yminHist,ymaxHist), breaks = c(), expand = c(0,0,0,1)) +
...
hist6 <- hist6 + theme(plot.margin = margin(0,0,0,0))
plot6 <- plot6 + scale_x_discrete(expand = c(0,.25,0,1))
plot6 <- plot6 + annotation_custom(ggplotGrob(hist6),
                                   xmin = xminInlay, xmax = xmaxInlay,
                                   ymin = yminInlay, ymax = ymaxInlay)

hist857 <- hist857 + theme(plot.margin = margin(0,0,0,0))
plot857 <- plot857 + scale_x_discrete(expand = c(0,.25,0,1))
plot857 <- plot857 + annotation_custom(ggplotGrob(hist857),
                                       xmin = xminInlay, xmax = xmaxInlay,
                                       ymin = yminInlay, ymax = ymaxInlay)

hist15 <- hist15 + theme(plot.margin = margin(0,0,0,0))
plot15 <- plot15 + scale_x_discrete(expand = c(0,.25,0,1))
plot15 <- plot15 + annotation_custom(ggplotGrob(hist15),
                                     xmin = xminInlay, xmax = xmaxInlay,
                                     ymin = yminInlay, ymax = ymaxInlay)
...
plot6 <- plot6 + theme(plot.margin = margin(10,5,10,5))
plot857 <- plot857 + theme(plot.margin = margin(10,5,10,5))
plot15 <- plot15 + theme(plot.margin = margin(10,5,10,5))

# join plots and show them in RStudio
jointPlots <- ggarrange(plot6, plot857, plot15,
                        nrow = 1, ncol = 3,
                        font.label = list(size = 20, family = fType))
jointPlots

# Save plot as jpg and pdf
savename = paste0(parentFolder,"/figures/36_supp_means_PLI_jointLabs.pdf")
ggsave(filename = savename, plot = jointPlots, device = "pdf",
       width = 21, height = 7, unit = "cm", limitsize = FALSE)

savename = paste0(parentFolder,"/figures/36_supp_means_PLI_jointLabs.jpg")
ggsave(filename = savename, plot = jointPlots, device = "jpg",
       width = 21, height = 7, unit = "cm", dpi = 300, limitsize = FALSE)

...
# --- content: reading individual spectra and creating data frames for statistics & plots (common pipeline, single-trial analyses)
...
fileFolder <- paste0(parentFolder,"/individualSpectra/commonPipeline_singleTrialFFT/")
...
savename = paste0(parentFolder,"/dataframes/supp_dfSSVEP_commPL_singleTrialFFT.csv")
...
nrBins6 <- length(specAvgFL6sq)
nrBins857 <- length(specAvgFL857sq)
nrBins15 <- length(specAvgFL15sq)
...
dfSpectra <- data.frame(
  lab = factor(c(rep(1, 2*(nrBins6+nrBins857+nrBins15)), rep(2, 2*(nrBins6+nrBins857+nrBins15))),
               levels = c(1,2), labels = c("Florida","Leipzig")),
  freq = factor(rep(c(rep(1, 2*nrBins6), rep(2, 2*nrBins857), rep(3, 2*nrBins15)), 2),
                levels = c(1,2,3), labels = c("6Hz", "8.57Hz", "15Hz")),
  mod = factor(rep(c(rep(1, nrBins6), rep(2, nrBins6),
                     rep(1, nrBins857), rep(2, nrBins857),
                     rep(1, nrBins15), rep(2, nrBins15)), 2),
               levels = c(1,2), labels = c("square", "sine")),
  freqBin = rep(c(rep(seq(0, (nrBins6-1) * freqRes6, freqRes6), 2),
                  rep(seq(0, (nrBins857-1) * freqRes857, freqRes857), 2),
                  rep(seq(0, (nrBins15-1) * freqRes15, freqRes15), 2)), 2),
...
dfSpectraJoint <- data.frame(
  freq = factor(c(rep(1, 2*nrBins6), rep(2, 2*nrBins857), rep(3, 2*nrBins15)),
                levels = c(1,2,3), labels = c("6Hz", "8.57Hz", "15Hz")),
  mod = factor(c(rep(1, nrBins6), rep(2, nrBins6),
                 rep(1, nrBins857), rep(2, nrBins857), 
                 rep(1, nrBins15), rep(2, nrBins15)),
               levels = c(1,2), labels = c("square", "sine")),
  freqBin = c(rep(seq(0, (nrBins6-1) * freqRes6, freqRes6), 2),
              rep(seq(0, (nrBins857-1) * freqRes857, freqRes857), 2),
              rep(seq(0, (nrBins15-1) * freqRes15, freqRes15), 2)),
  amp = c(specAvg6sq, specAvg6sin, specAvg857sq, specAvg857sin, specAvg15sq, specAvg15sin)
)

# Save data frames
savename = paste0(parentFolder,"/dataframes/supp_dfSpectra_commPL_singleTrialFFT.csv")
write.csv(x = dfSpectra, savename, row.names = FALSE)

savename = paste0(parentFolder,"/dataframes/supp_dfSpectra_commPL_singleTrialFFT_jointLabs.csv")
...
# save topo dataframe
savename = paste0(parentFolder,"/dataframes/supp_dfTopos_commPL_singleTrialFFT.csv")

...
# --- content: pairwise tests for ssVEP data from common pipeline
...
t_test(dfFreqEffects, ssvep ~ freq, paired = TRUE, p.adjust.method = "none")
cohens_d(dfFreqEffects, ssvep ~ freq, paired = TRUE)
ttestBF(x = dfFreqEffects$ssvep[dfFreqEffects$freq == "6Hz"],
        y = dfFreqEffects$ssvep[dfFreqEffects$freq == "8.57Hz"],
        nullInterval = NULL, paired = TRUE);
ttestBF(x = dfFreqEffects$ssvep[dfFreqEffects$freq == "6Hz"],
        y = dfFreqEffects$ssvep[dfFreqEffects$freq == "15Hz"],
        nullInterval = NULL, paired = TRUE);
ttestBF(x = dfFreqEffects$ssvep[dfFreqEffects$freq == "8.57Hz"],
        y = dfFreqEffects$ssvep[dfFreqEffects$freq == "15Hz"],
        nullInterval = NULL, paired = TRUE);

# Main Effect Mod Across Samples
t_test(dfModEffects, ssvep ~ mod, paired = TRUE, p.adjust.method = "none")
cohens_d(dfModEffects, ssvep ~ mod, paired = TRUE)
ttestBF(x = dfModEffects$ssvep[dfModEffects$mod == "box"],
        y = dfModEffects$ssvep[dfModEffects$mod == "sine"],
        nullInterval = NULL, paired = TRUE);
...
ttestBF(x = df$ssvepZ[df$freq == "6Hz" & df$mod == "box"],
        y = df$ssvepZ[df$freq == "6Hz" & df$mod == "sine"],
        nullInterval = NULL, paired = TRUE)
ttestBF(x = df$ssvepZ[df$freq == "8.57Hz" & df$mod == "box"],
        y = df$ssvepZ[df$freq == "8.57Hz" & df$mod == "sine"],
        nullInterval = NULL, paired = TRUE)
ttestBF(x = df$ssvepZ[df$freq == "15Hz" & df$mod == "box"],

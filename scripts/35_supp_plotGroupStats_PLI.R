...
# --- content: plotting PLI group stats for different conditions and labs (common pipeline)
...
dfStatsDiff <- pivot_wider(data = dfStats, id_cols = c(part,lab,freq), names_from = c(mod), values_from = c(pliZ))
...
savename = paste0(parentFolder,"/figures/33_supp_means_PLI.pdf")
ggsave(filename = savename, plot = meanPlots, device = "pdf",
       width = 21, height = 15, unit = "cm", limitsize = FALSE)

savename = paste0(parentFolder,"/figures/33_supp_means_PLI.jpg")

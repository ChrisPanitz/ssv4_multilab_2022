...
# --- content: plotting spectra for different conditions (different pipelines)
...
specFL6 <- ggplot(data = dfSpectra[dfSpectra$lab == "Florida" & dfSpectra$freq == "6Hz",], 
  ...
  scale_y_continuous(name = "Spectral amplitude (µV/Hz)", limits = c(yminFL,ymaxFL)) +
...
specLE6 <- ggplot(data = dfSpectra[dfSpectra$lab == "Leipzig" & dfSpectra$freq == "6Hz",], 
                  aes(x = freqBin, y = amp, fill = mod)) + theme_classic() +
  geom_area(data = dfSpectra[dfSpectra$lab == "Leipzig" & dfSpectra$freq == "6Hz" & dfSpectra$mod == "square",]) +
  geom_area(data = dfSpectra[dfSpectra$lab == "Leipzig" & dfSpectra$freq == "6Hz" & dfSpectra$mod == "sine",],
            alpha = .80) +
  #geom_vline(xintercept = 6, color = "gray70", linetype = "dashed", size = .2) +
  scale_x_continuous(name = " ", limits = c(xmin,xmax), breaks = c(6,8.57,15), labels = c("6","8.57","15")) +
  scale_y_continuous(name = "Spectral amplitude (µV/cm^2)", limits = c(yminLE,ymaxLE)) +
  scale_fill_manual(values = brewer.pal(n = 4, "Purples")[4:3], breaks = c("square","sine")) +
...
specLE857 <- ggplot(data = dfSpectra[dfSpectra$lab == "Leipzig" & dfSpectra$freq == "8.57Hz",], 
                    aes(x = freqBin, y = amp, fill = mod)) + theme_classic() +
  geom_area(data = dfSpectra[dfSpectra$lab == "Leipzig" & dfSpectra$freq == "8.57Hz" & dfSpectra$mod == "square",]) +
  geom_area(data = dfSpectra[dfSpectra$lab == "Leipzig" & dfSpectra$freq == "8.57Hz" & dfSpectra$mod == "sine",],
            alpha = .80) +
  #geom_vline(xintercept = 60/7, color = "gray70", linetype = "dashed", size = .2) +
...
specLE15 <- ggplot(data = dfSpectra[dfSpectra$lab == "Leipzig" & dfSpectra$freq == "15Hz",], 
                   aes(x = freqBin, y = amp, fill = mod)) + theme_classic() +
  geom_area(data = dfSpectra[dfSpectra$lab == "Leipzig" & dfSpectra$freq == "15Hz" & dfSpectra$mod == "square",]) +
  geom_area(data = dfSpectra[dfSpectra$lab == "Leipzig" & dfSpectra$freq == "15Hz" & dfSpectra$mod == "sine",],
            alpha = .80) +
  #geom_vline(xintercept = 15, color = "gray70", linetype = "dashed", size = .2) +
  scale_x_continuous(name = " ", limits = c(xmin,xmax), breaks = c(6,8.57,15), labels = c("6","8.57","15")) +
  scale_y_continuous(name = " ", limits = c(yminLE,ymaxLE)) +

...
# --- content: plotting group stats for different conditions and labs (common pipeline, single-trial FFT)
...
for (i in 1:length(levels(dfStats$part))) {
  dfStatsWithin$ssvepZ[dfStats$part == levels(dfStats$part)[i]] <- 
...
dfStatsDiff <- pivot_wider(data = dfStats, id_cols = c(part,lab,freq), names_from = c(mod), values_from = c(ssvepZ))
...
savename = paste0(parentFolder,"/figures/23_supp_means_commPL_singleTrialFFT.pdf")
ggsave(filename = savename, plot = meanPlots, device = "pdf",
       width = 21, height = 15, unit = "cm", limitsize = FALSE)

savename = paste0(parentFolder,"/figures/23_supp_means_commPL_singleTrialFFT.jpg")

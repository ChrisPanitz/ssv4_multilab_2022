...
# --- content: plotting group stats (amplitude and SNR) for different conditions in aggregated sample (common pipeline, single-trial FFT)
...
loadname <- paste0(parentFolder,"/dataframes/supp_dfSpectra_commPL_singleTrialFFT_jointLabs.csv")
...
spec6 <- ggplot(data = dfSpectra[dfSpectra$freq == "6Hz",], 
  ...
  scale_x_continuous(name = "Frequency (Hz)", limits = c(0,3.5*6), breaks = seq(6,3*6,6), labels = as.character(seq(6,3*6,6))) +
  scale_y_continuous(name = "Spectral amplitude ([µV/rad^2] / Hz)", limits = c(yminSpec,ymaxSpec)) +
...
spec857 <- ggplot(data = dfSpectra[dfSpectra$freq == "8.57Hz",], 
  ...
  scale_x_continuous(name = "Frequency (Hz)", limits = c(0,3.5*8.57), breaks = seq(8.57,3*8.57,8.57), labels = as.character(seq(8.57,3*8.57,8.57))) +
...
spec15 <- ggplot(data = dfSpectra[dfSpectra$freq == "15Hz",], 
  ...
  scale_x_continuous(name = "Frequency (Hz)", limits = c(0,3.5*15), breaks = seq(15,3*15,15), labels = as.character(seq(15,3*15,15))) +
  scale_y_continuous(name = " ", limits = c(yminSpec,ymaxSpec)) +
...
yminHist <- 0
ymaxHist <- 16 # set to NA for automatic scaling
inlayFill = "gray80"
...
savename = paste0(parentFolder,"/figures/26_supp_means_commPL_singleTrialFFT_jointLabs.pdf")
ggsave(filename = savename, plot = jointPlots, device = "pdf",
       width = 21, height = 23, unit = "cm", limitsize = FALSE)

savename = paste0(parentFolder,"/figures/26_supp_means_commPL_singleTrialFFT_jointLabs.jpg")

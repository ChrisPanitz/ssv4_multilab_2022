...
# --- content: plotting SNR group stats for different conditions (common pipeline)
...
for (i in 1:length(levels(dfStats$part))) {
  dfStatsWithin$ssvepSNR_Z[dfStats$part == levels(dfStats$part)[i]] <- dfStats$ssvepSNR_Z[dfStats$part == levels(dfStats$part)[i]] - mean(dfStats$ssvepSNR_Z[dfStats$part == levels(dfStats$part)[i]]) + mean(dfStats$ssvepSNR_Z)
}
...
savename = paste0(parentFolder,"/figures/13b_means_commPL_SNR.pdf")
ggsave(filename = savename, plot = meanPlots, device = "pdf",
       width = 21, height = 15, unit = "cm", limitsize = FALSE)

savename = paste0(parentFolder,"/figures/13b_means_commPL_SNR.jpg")
